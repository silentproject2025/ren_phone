...
void setup(){
  ...
  // v27: PERBESAR PEMAKAIAN PSRAM utk alokasi umum (String, buffer
  // HTTPClient/WiFiClientSecure, dll yg SEBELUMNYA otomatis jatuh ke
  // RAM internal walau PSRAM masih longgar). AKAR MASALAH error
  // "RAM internal terlalu terfragmentasi/rendah utk TLS" (lihat cek
  // largestBlock di triggerGeminiAI()): banyak alokasi kecil-menengah
  // (String concat buat JSON request, dll) numpuk di RAM internal yg
  // cuma ~300KB & dipakai bareng WiFi/BT stack, bikin gampang
  // terfragmentasi. heap_caps_malloc_extmem_enable(N) bikin malloc()/
  // `new`/String yg ukurannya >= N byte DICOBA dulu ke PSRAM (yg jauh
  // lebih lapang, biasanya 2-8MB) sblm jatuh ke internal -- jadi RAM
  // internal lebih longgar & gak gampang fragmented pas giliran TLS
  // butuh alokasi gedenya sendiri. Nilai 2048 dipilih supaya buffer
  // kecil (variabel lokal, dll) yg emang HARUS cepat/DMA-capable tetap
  // di internal, tapi apapun yg lebih gede dari itu diarahkan ke PSRAM.
  if(psramFound()){
    heap_caps_malloc_extmem_enable(2048);
    Serial.printf("[Boot] PSRAM ditemukan (%u bytes), extmem_enable threshold=2048\n", ESP.getPsramSize());
  } else {
