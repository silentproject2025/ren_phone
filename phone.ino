...
// v33 FIX: "EspUsbHost.h" WAJIB di-include di GRUP PALING ATAS ini, BUKAN
// persis di atas kode USB Host di bawah (tempat semula). Arduino/arduino-cli
// men-generate prototype function OTOMATIS (usbOnMouse, usbOnKeyboard, dst)
// dan menyisipkannya di AWAL file -- SEBELUM baris #include yang berada di
// tengah file. Akibatnya tipe EspUsbHostMouseEvent/EspUsbHostKeyboardEvent/
// EspUsbHostDeviceInfo belum dikenal saat prototype itu disisipkan -> error
// "does not name a type" saat compile (persis error yg dilaporkan). Include
// di sini (paling atas, sebelum kode apapun) menjamin tipe-tipe itu sudah
// dikenal sebelum prototype otomatis dibuat.
...
void sendGeminiRequest(String promptText) {
  ...
  // v27: kasih jeda singkat SETELAH koneksi ditutup (client/http di
  ...
  // butuh sedikit waktu nyata buat beres2 (TCP FIN/ACK, ngelepas
  // struktur socket internal) SETELAH sebuah koneksi ditutup -- kalau
  // user langsung ngirim pertanyaan kedua detik itu juga, pembersihan
  // itu mungkin belum kelar, jd largestBlock RAM internal pas dicek lg
  // di triggerGeminiAI() masih keitung "kepake" walau sbnrnya udah mau
  // dilepas. CATATAN JUJUR: ini upaya mitigasi yg masuk akal, BUKAN
  // fix yg udah kebukti manjur -- kalau masih kejadian, cek log Serial
  // "[Gemini] Sebelum bikin task -> freeInternal=... largestBlock=..."
  // pas percobaan ke-1 (sukses) vs ke-2 (gagal) buat liat SEBERAPA JAUH
  // bedanya, itu bakal ngasih tau apa ini beneran soal timing beres2
  // koneksi, atau ada penyebab lain yg belum ketauan.
  delay(500);
...
void triggerGeminiAI() {
  ...
  // v28: cek presisi dipindah ke doGeminiHttpRequest() yg jalan di
  // background task. Cek di titik INI sengaja dibikin longgar -- cuma
  // nolak bikin task sama sekali kalau RAM beneran kritis, bukan di
  // angka pas-pasan.
  // v30: ambang di doGeminiHttpRequest() diturunin 46000->20000 (lihat
  // catatan v30 di sana) -- sekarang SAMA dgn ambang di titik ini, krn
  // retry-loop yg tadinya beda alasannya (nunggu lwIP) TERBUKTI gak
  // ngefek sama sekali (retry di dalam task yg stack-nya sendiri yg
  // motong RAM -- gak mungkin pulih sambil nunggu di situ jg).
  if (largestBlock < 20000) {
    aiResponse = "Error: RAM internal kritis rendah. Restart perangkat.";
    needRedrawNow();
    return;
  }
...
void mjpegPlayFile(const String& filename){
  ...
  while(!mjpegStopRequested && mjFile.available() && mjpeg.readMjpegBuf()){
    ...
    if(++frameCheckCounter >= 2){
      ...
      if(getInputPoint(tp)){ // v33: klik mouse USB juga bisa hentikan playback
...
// PUSH FRAME
// =============================================
void push(){ drawUsbMouseCursor(canvas); canvas.pushSprite(0,0); }
...
void aviPlayFile(const String& filename){
  ...
  while(!aviStopRequested && (uint32_t)aviFile.position() < aviMoviEnd && aviFile.available()){
    ...
    if(++frameCheckCounter>=2){
      frameCheckCounter=0;
      lgfx::touch_point_t tp;
      if(getInputPoint(tp)) aviStopRequested=true; // v33: klik mouse USB juga bisa hentikan playback
    }
    if(wifiConnected && webServerRunning) webServer.handleClient();
    yield();
  }
...
// =============================================
// USB HOST: MOUSE & KEYBOARD (BARU v33)
// Pin GPIO 19 (D-) & GPIO 20 (D+) yg dipakai user buat OTG adalah PIN USB
// NATIVE bawaan ESP32-S3 (bukan GPIO sembarang) -- jadi dipakai library
// "EspUsbHost" (tanakamasayuki/EspUsbHost, API v2.x, butuh arduino-esp32
// core >=3.2.0) yg jalan lewat peripheral USB OTG asli chip ini, BUKAN
// soft-host bit-banging via GPIO umum (itu pendekatan beda utk board yg
// gak punya USB OTG native, gak relevan di sini).
//
// PENTING soal daya: banyak dev-board ESP32-S3 (termasuk DevKitC-1 resmi)
// TIDAK mengalirkan VBUS 5V ke device USB yg disambung saat mode host --
// kalau mouse/keyboard gak kedeteksi sama sekali, cek dulu apakah port OTG
// kamu memang mengalirkan 5V ke device (butuh suplai 5V terpisah kalau
// tidak).
//
// Event mouse/keyboard diproses EspUsbHost di FreeRTOS task LATAR
// BELAKANG-nya sendiri (bukan di loop() kita) -- makanya perubahan state
// di callback di bawah cuma nyentuh variabel global sederhana (posisi
// kursor, tombol, target teks keyboard), persis pola yg sudah dipakai utk
// idle-hook HWmonitor & task rekam mic: "murah & good-enough", bukan
// disinkronkan pakai mutex (konsisten sama gaya konkurensi proyek ini).
// (Catatan: #include "EspUsbHost.h" SUDAH dipindah ke grup #include paling
// atas file -- lihat komentar v33 FIX di sana. Jangan taruh include-nya
// balik ke sini, itu penyebab error compile "does not name a type".)
// =============================================

EspUsbHost usb;

// ---- Mouse: disimulasikan jadi 1 titik "sentuh" sintetis ----
// Kenapa didesain begini (bukan API terpisah)? Supaya SELURUH pipeline
// yang sudah ada -- swipe gesture, lock screen, Control Center, keyboard
// virtual, tombol Reset di HWmonitor, dst -- otomatis jalan buat mouse
// juga TANPA nyentuh kode masing-masing app. Lihat getInputPoint() di
// bawah: dipanggil gantiin display.getTouch() langsung di loop().
volatile bool    usbMouseConnected = false;
volatile float   usbMouseX = SCR_W/2.0f, usbMouseY = SCR_H/2.0f;
volatile uint8_t usbMouseButtons = 0; // bit0 = tombol kiri (konvensi HID standar)
#define USB_MOUSE_LEFT_BIT 0x01
// Sensitivitas gerak mouse (px layar per "count" HID mentah). DPI mouse
// beda-beda banget antar merek -- kalau kursor kerasa kelewat cepat/lambat
// di layar 240x320/320x240 ini, sesuaikan angka ini duluan sebelum ubah
// yang lain.
#define USB_MOUSE_SENSITIVITY 1.0f

void usbOnMouse(const EspUsbHostMouseEvent &event){
  usbMouseConnected = true;
  usbMouseX = constrain(usbMouseX + event.x*USB_MOUSE_SENSITIVITY, 0, SCR_W-1);
  usbMouseY = constrain(usbMouseY + event.y*USB_MOUSE_SENSITIVITY, 0, SCR_H-1);
  usbMouseButtons = event.buttons;
  needRedraw = true; // biar kursornya ikut update posisi walau gak lagi diklik
}

// Dipanggil dari loop() gantiin display.getTouch(&tp) langsung -- lihat
// perubahan di LOOP UTAMA. Prioritas: mouse dipakai HANYA selagi tombol
// kiri ditekan (touchscreen beneran juga cuma "aktif" pas disentuh, bukan
// pas jari melayang) -- kalau enggak, jatuh balik ke touchscreen fisik
// seperti biasa, jadi keduanya tetap bisa dipakai gantian kapan saja.
bool getInputPoint(lgfx::touch_point_t &tp){
  if(usbMouseConnected && (usbMouseButtons & USB_MOUSE_LEFT_BIT)){
    tp.x = (int16_t)usbMouseX;
    tp.y = (int16_t)usbMouseY;
    return true;
  }
  return display.getTouch(&tp);
}

// Kursor panah kecil, digambar di FRAME FINAL (dalam push(), bukan di
// masing-masing draw*() app) -- supaya nongol di atas layar apa pun tanpa
// perlu disentuh satu-satu per app. Putih+outline hitam biar kebaca di
// tema apa pun (Dark/AMOLED/Light/Pastel).
void drawUsbMouseCursor(LGFX_Sprite& s){
  if(!usbMouseConnected) return;
  int cx=(int)usbMouseX, cy=(int)usbMouseY;
  s.fillTriangle(cx,cy, cx,cy+16, cx+11,cy+11, 0x0000);
  s.fillTriangle(cx+1,cy+2, cx+1,cy+13, cx+9,cy+10, 0xFFFF);
}

// ---- Keyboard: nyambung ke target field yg sama dipakai keyboard virtual ----
// kbVisible/kbTarget (String*) adalah abstraksi yang SAMA dipakai keyboard
// virtual on-screen (lihat kbTouch() di bagian VIRTUAL KEYBOARD) buat
// nunjuk field aktif -- settSSID, settPass, noteText, aiPrompt, updUrl,
// imgLibSearchInput, dll. Jadi keyboard fisik otomatis nyambung ke field
// yang sama, di app manapun, tanpa perlu tau app mana yg lagi aktif.
#define USB_HID_KEYCODE_ENTER     0x28 // USB HID usage: Keyboard Return
#define USB_HID_KEYCODE_BACKSPACE 0x2A // USB HID usage: Keyboard Backspace

void usbOnKeyboard(const EspUsbHostKeyboardEvent &event){
  if(!event.pressed) return;       // cuma proses saat KETUKAN turun, bukan lepas
  if(!kbVisible || !kbTarget) return; // sama kayak keyboard virtual: aktif cuma kalau ada field teks yg fokus

  if(event.keycode == USB_HID_KEYCODE_BACKSPACE){
    if(kbTarget->length()>0) *kbTarget = kbTarget->substring(0, kbTarget->length()-1);
    needRedraw = true;
    return;
  }
  if(event.keycode == USB_HID_KEYCODE_ENTER){
    // Enter = sama persis kayak ketuk di LUAR keyboard virtual: nutup
    // keyboard (satu-satunya cara "commit" field yg sudah ada sebelumnya).
    kbVisible = false; kbTarget = nullptr;
    needRedraw = true;
    return;
  }
  // Karakter cetak biasa -- EspUsbHost sudah ngurus shift/layout sendiri
  // lewat field `ascii`, jadi tinggal nempel langsung ke target.
  if(event.ascii >= 0x20 && event.ascii <= 0x7E){
    *kbTarget += (char)event.ascii;
    needRedraw = true;
  }
}

void usbOnDeviceDisconnected(const EspUsbHostDeviceInfo &device){
  // Simplifikasi yg disengaja: begitu ADA perangkat USB apapun yg dicabut,
  // kursor mouse disembunyikan. Cukup buat kasus 1 mouse ATAU 1 keyboard
  // tersambung gantian -- kalau kamu pasang HUB dgn mouse+keyboard SEKALIGUS,
  // ini bisa salah nyembunyiin kursor pas yg dicabut cuma keyboard-nya.
...
void setup(){
  ...
  // v33: USB Host mouse & keyboard lewat pin native GPIO 19/20. Ini
  // peripheral OPSIONAL -- kalau usb.begin() gagal (mis. sebelum ada
  // device tersambung), booting TETAP lanjut normal, cuma fitur mouse/
  // keyboard-nya belum aktif sampai ada perangkat dicolok & di-retry
  // sendiri oleh library di background.
  usb.onMouse(usbOnMouse);
  usb.onKeyboard(usbOnKeyboard);
  usb.onDeviceDisconnected(usbOnDeviceDisconnected);
  if(!usb.begin()){
    Serial.printf("USB Host init gagal: %s\n", usb.lastErrorName());
  }
...
void loop(){
  ...
  lgfx::touch_point_t tp;
  bool touched=getInputPoint(tp); // v33: gabungan touchscreen fisik + mouse USB (lihat getInputPoint())
