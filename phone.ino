...
bool apodDecodeCachedImage(){
  ...
  if(!apodJpeg.open(f, apodJpegDrawCallback)){
    f.close();
...
void apodFetchAndProcess(String dateParam){
  ...
  apodImgReady=false;
  if(apodMediaType=="image" && imgUrl.length() && sdReady){
  ...
  if(sdReady && apodMediaType=="image" && SD_MMC.exists(apodImgPath(apodCurDate))) apodDecodeCachedImage();
...
void drawApod(LGFX_Sprite& s){
  ...
  // v22: ikon kaca pembesar cari-tanggal, pola sama dgn AI Chat (keyboard
  // digambar DI ATAS konten yg sudah ada, bukan ganti sprite dr awal).
  if(kbVisible){ drawKb(s); } else { drawSearchBar(s, apodSearchBar, apodSearchRightX(), apodSearchTopY(), apodSearchInput, "cth: 2026-08-23"); drawBack(s); }
  drawToast(s);
}
...
void apodTouch(int x,int y,bool held,bool isNew){
  ...
  if(isNew){
    ...
    if(hit==2){ kbTarget=&apodSearchInput; kbVisible=true; kbMode=KB_NUM; needRedraw=true; return; }
    if(hit==3){
      String q=apodSearchInput; q.trim();
      searchBarCollapse(apodSearchBar);
      if(q.length()) apodLoadFromCacheOrFetch(q);
      return;
    }
...
void neoFetchAndProcess(String dateParam){
  if(WiFi.status()!=WL_CONNECTED){
    ...
    neoPage=NEO_ERROR; needRedrawNow(); return;
  }
  neoEnsureCacheDir();
...
void neoStartFetch(String dateParam){
  ...
  if(res!=pdPASS){
    ...
    neoPage=NEO_ERROR; needRedraw=true;
  }
}
...
void neoEnter(){ neoLoadOrFetch(neoCurDate); }
void neoExit(){}
...
void drawNeoList(LGFX_Sprite& s){
  char hdr[48];
  snprintf(hdr,sizeof(hdr),"NEO Asteroid - %s",neoCurDate.c_str());
  s.setTextColor(T().accent); s.setTextSize(1); s.setCursor(8,26); s.print(hdr);
...
void neoTouch(int x,int y,bool held,bool isNew){
  ...
  if(hit==2){ kbTarget=&neoSearchInput; kbVisible=true; kbMode=KB_NUM; needRedraw=true; return; }
  if(hit==3){
...
bool marsDecodeCachedImage(long id){
  ...
  if(!marsJpeg.open(f,marsJpegDrawCallback)){ f.close(); marsImgReady=true; return false; }
...
void marsFetchAndProcess(String rover,int sol){
  if(WiFi.status()!=WL_CONNECTED){
    ...
    marsPage=MARS_ERROR; needRedrawNow(); return;
  }
  marsEnsureCacheDir();
  WiFiClientSecure client; client.setInsecure(); client.setTimeout(15000);
  HTTPClient http; http.setTimeout(15000); http.setConnectTimeout(15000);
  ...
  if(code!=HTTP_CODE_OK){
    ...
    marsErrorMsg="Gagal ambil foto Mars Rover (HTTP "+String(code)+")"+(msg.length()?(": "+msg):String("."));
    marsPage=MARS_ERROR; needRedrawNow(); return;
  }
  String arrBody = nasaExtractArrayByKey(body,"photos");
  ...
  if(marsCount==0){
    marsErrorMsg="Tidak ada foto ditemukan utk rover "+rover+" pd sol "+String(sol)+".";
    marsPage=MARS_ERROR; needRedrawNow(); return;
  }
  marsRoverName=rover; marsSol=sol;
  marsSaveCache(rover,sol);
  ...
  {
    MarsPhoto& ph=marsList[0];
    String path=marsImgPath(ph.id);
    if(sdReady){
      if(!SD_MMC.exists(path)) apodDownloadImage(ph.imgSrc,path);
      if(SD_MMC.exists(path)) marsDecodeCachedImage(ph.id); else marsImgReady=true;
    } else marsImgReady=true;
  }
  marsPage=MARS_VIEW;
  needRedrawNow();
}
...
void marsLoadPhotoTaskFunc(void* parameter){
  ...
  delete pp;
  String path=marsImgPath(id);
  if(!SD_MMC.exists(path)) apodDownloadImage(url,path);
  if(SD_MMC.exists(path)) marsDecodeCachedImage(id); else marsImgReady=true;
...
void marsEnter(){ marsLoadOrFetch(marsRoverName,marsSol); }
void marsExit(){}
...
void drawMarsView(LGFX_Sprite& s){
  ...
  if(marsPhotoLoading){
    ...
    s.setCursor(SCR_W/2-(int)strlen(m)*3,imgTop+imgH/2-4); s.print(m);
  } else if(marsImgReady && marsImgW==SCR_W && marsImgH==imgH && marsCount>0){
  ...
  char cnt[24]; snprintf(cnt,sizeof(cnt),"%d/%d",marsCount?marsIdx+1:0,marsCount);
...
void marsTouch(int x,int y,bool held,bool isNew){
  ...
  if(hit==2){ kbTarget=&marsSearchInput; kbVisible=true; kbMode=KB_NUM; needRedraw=true; return; }
  if(hit==3){
    String q=marsSearchInput; q.trim();
    searchBarCollapse(marsSearchBar);
    int sol=q.toInt();
    if(sol>0 || q=="0") marsLoadOrFetch(marsRoverName,sol);
    return;
  }
...
bool epicDecodeCachedImage(const String& imageName){
  ...
  if(!epicJpeg.open(f,epicJpegDrawCallback)){ f.close(); epicImgReady=true; return false; }
...
void epicFetchAndProcess(String dateParam){
  if(WiFi.status()!=WL_CONNECTED){
    ...
    epicPage=EPIC_ERROR; needRedrawNow(); return;
  }
  epicEnsureCacheDir();
  WiFiClientSecure client; client.setInsecure(); client.setTimeout(15000);
  HTTPClient http; http.setTimeout(15000); http.setConnectTimeout(15000);
  ...
  epicSaveCache(usedDate);
  epicIdx=0; epicImgReady=false; epicPhotoLoading=false;
  {
    EpicItem& it=epicList[0];
    String path=epicImgPath(it.image);
    String iurl=epicArchiveUrl(usedDate,it.image);
    if(sdReady){
      if(!SD_MMC.exists(path)) apodDownloadImage(iurl,path);
      if(SD_MMC.exists(path)) epicDecodeCachedImage(it.image); else epicImgReady=true;
    } else epicImgReady=true;
  }
  epicPage=EPIC_VIEW;
  needRedrawNow();
}
...
void epicStartFetch(String dateParam){
  ...
  if(res!=pdPASS){
    ...
    epicPage=EPIC_ERROR; needRedraw=true;
  }
}
...
void epicLoadPhotoTaskFunc(void* parameter){
  ...
  String url=pp->substring(sep+1);
  delete pp;
  String path=epicImgPath(img);
  if(!SD_MMC.exists(path)) apodDownloadImage(url,path);
  if(SD_MMC.exists(path)) epicDecodeCachedImage(img); else epicImgReady=true;
...
void epicLoadOrFetch(String dateParam){
  if(epicTaskHandle!=NULL) return;
  if(dateParam.length()==10 && epicLoadCache(dateParam)){
...
void epicEnter(){ epicLoadOrFetch(epicCurDate); }
void epicExit(){}
...
void drawEpicView(LGFX_Sprite& s){
  char hdr[40]; snprintf(hdr,sizeof(hdr),"Bumi EPIC - %s",epicCurDate.c_str());
  s.setTextColor(T().accent); s.setTextSize(1); s.setCursor(8,26); s.print(hdr);
  ...
  if(epicPhotoLoading){
    ...
    s.setCursor(SCR_W/2-(int)strlen(m)*3,imgTop+imgH/2-4); s.print(m);
  } else if(epicImgReady && epicImgW==SCR_W && epicImgH==imgH && epicCount>0){
  ...
  s.fillRoundRect(6,navY,50,20,5,T().surface2); s.setTextColor(T().accent); s.setCursor(16,navY+5); s.print("<");
  s.fillRoundRect(SCR_W-56,navY,50,20,5,T().surface2); s.setTextColor(T().accent); s.setCursor(SCR_W-46,navY+5); s.print(">");
  char cnt[24]; snprintf(cnt,sizeof(cnt),"%d/%d",epicCount?epicIdx+1:0,epicCount);
  s.setTextColor(T().text); s.setCursor(SCR_W/2-(int)strlen(cnt)*3,navY+5); s.print(cnt);
...
void epicTouch(int x,int y,bool held,bool isNew){
  ...
  if(hit==2){ kbTarget=&epicSearchInput; kbVisible=true; kbMode=KB_NUM; needRedraw=true; return; }
  if(hit==3){
...
bool imgLibDecodeCachedImage(const String& nasaId){
  ...
  if(!imgLibJpeg.open(f,imgLibJpegDrawCallback)){ f.close(); imgLibImgReady=true; return false; }
...
void imgLibFetchAndProcess(String query){
  if(WiFi.status()!=WL_CONNECTED){
    ...
    imgLibPage=ILIB_ERROR; needRedrawNow(); return;
  }
  imgLibEnsureCacheDir();
  WiFiClientSecure client; client.setInsecure(); client.setTimeout(15000);
  HTTPClient http; http.setTimeout(15000); http.setConnectTimeout(15000);
  ...
  imgLibQuery=query;
  imgLibSaveCache(query);
  imgLibIdx=0; imgLibImgReady=false; imgLibPhotoLoading=false;
  {
    ImgLibItem& it=imgLibList[0];
    if(it.thumbUrl.length() && sdReady){
      String path=imgLibImgPath(it.nasaId);
      if(!SD_MMC.exists(path)) apodDownloadImage(it.thumbUrl,path);
      if(SD_MMC.exists(path)) imgLibDecodeCachedImage(it.nasaId); else imgLibImgReady=true;
    } else imgLibImgReady=true;
  }
...
void imgLibStartFetch(String query){
  ...
  if(res!=pdPASS){
    ...
    imgLibPage=ILIB_ERROR; needRedraw=true;
  }
}
...
void imgLibLoadPhotoTaskFunc(void* parameter){
  ...
  if(url.length()){
    if(!SD_MMC.exists(path)) apodDownloadImage(url,path);
    if(SD_MMC.exists(path)) imgLibDecodeCachedImage(nasaId); else imgLibImgReady=true;
  } else imgLibImgReady=true;
...
void imgLibEnter(){ if(imgLibQuery.length()) imgLibLoadOrFetch(imgLibQuery); }
void imgLibExit(){}
...
void imgLibTouch(int x,int y,bool held,bool isNew){
  ...
  if(hit==3){
    ...
    if(q.length()) imgLibLoadOrFetch(q);
    return;
  }
