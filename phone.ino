...
void vibTap(){       // ketuk tombol/ikon biasa di mana pun -- getar pendek & halus
  static const VibStep p[] = {{1,16},{0,1}};
  vibPlayPattern(p,2);
}
void vibNav(){        // pindah layar / buka app / kembali (Back/Home) -- 1 getar agak lbh terasa
  static const VibStep p[] = {{1,30},{0,1}};
  vibPlayPattern(p,2);
}
void vibSuccess(){    // berhasil (data/gambar selesai dimuat, hasil cari ketemu) -- 2 getar pendek cepat
  static const VibStep p[] = {{1,20},{0,45},{1,20},{0,1}};
  vibPlayPattern(p,4);
}
void vibError(){       // gagal/error (fetch gagal, HTTP error, dst) -- 1 getar panjang & tegas
  static const VibStep p[] = {{1,140},{0,1}};
  vibPlayPattern(p,2);
}
void vibWarn(){        // peringatan ringan (input kosong/gak valid, dsb) -- 2 getar pendek pelan berjeda
  static const VibStep p[] = {{1,14},{0,60},{1,14},{0,1}};
  vibPlayPattern(p,4);
}
...
void navPush(Screen s){
  ...
  vibNav(); // v23: getar dikit tiap buka app baru
