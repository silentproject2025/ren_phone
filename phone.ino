...
bool doGeminiHttpRequest(const String& promptText, String& outResponse) {
  ...
  // beneran nyerah.
  const size_t MIN_BLOCK_FOR_TLS = 46000;
  size_t lb = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL);
  for (int i = 0; i < 5 && lb < MIN_BLOCK_FOR_TLS; i++) {
    delay(300);
    lb = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL);
    Serial.printf("[Gemini] largestBlock msh kurang (%u), retry ke-%d...\n", lb, i + 1);
  }
...
void triggerGeminiAI() {
  ...
  // v28: cek presisi (ambang 46000 + retry) dipindah ke doGeminiHttpRequest()
  // yg jalan di background task, supaya kalau largestBlock cuma butuh
  // sepersekian detik lagi buat pulih (lwIP blm sempet beres2 dr koneksi
  // sblmnya), UI DI SINI GAK IKUT NUNGGU/FREEZE. Cek di titik ini sengaja
  // dibikin longgar -- cuma nolak bikin task sama sekali kalau RAM
  // beneran kritis (jauh di bawah kebutuhan TLS manapun), bukan di angka
  // pas-pasan yg masih punya peluang pulih sebentar lagi.
