...
void drawControlCenter(LGFX_Sprite& s){
  ...
  const char* labels[CC_ITEMS]={
    "WiFi", "Airplane", "DND", "Tema", "Orientasi", "Shake", "Kunci", "Neopixel"
  };
  bool activeState[CC_ITEMS]={wifiConnected,airplaneMode,dndMode,false,false,shakeEnabled,false,npxOn};
...
void ccTouch(int x,int y){
  ...
  int cw=ccCardW(), ch=ccCardH(), gap=ccGap(), top=ccGridTop(), gx=ccGridX();
  void(*actions[CC_ITEMS])() = { ccActWifi, ccActAirplane, ccActDnd, ccActTheme, ccActOrient, ccActShake, nullptr, ccActNeopixel };
  for(int i=0;i<CC_ITEMS;i++){
    int col=i%CC_COLS, row=i/CC_COLS;
...
// =================================================================
// v26: BOOT SEQUENCE DIGANTI TOTAL -- rebrand dari "Ren Phone"/"SanzX OS"
// ke "Accretion"/"NyxOS", tema accretion disk / black hole. Fungsi
// bantu (navEase, lerpColor565, bootDrawGradientBg, push(), bootFade,
// palet BOOT_*) DIPAKAI ULANG PERSIS dr boot lama -- gak ada
// alpha-blending baru yg berat, animasi tetap pakai trik "cheap glow"
// yg sama (layered fillCircle di-lerp ke warna background, lihat
// bootDrawGlowDot). BEDA dr boot lama: tiap tahap dipacu BERBASIS WAKTU
// (millis()) drpd frame-count tetap (STEPS+delay(n) tetap) -- durasi
// tiap tahap jadi konsisten walau 1 frame kebetulan lebih lambat
// digambar (mis. pas SD card lg diakses barengan).
// bootStageRenPhone/bootStageSanzXOS/bootDrawParticleRing versi lama
// SUDAH TIDAK DIPAKAI (boleh dihapus scr terpisah kalau mau beres2,
// sengaja gak disentuh di sini spy diff minimal & gampang di-review).
// =================================================================
...
// SEBAGIAN area saja tanpa bikin "kotak hitam" yg warnanya beda dari
// gradient di sekelilingnya.
...
// Palet boot bersama (dipertahankan dr versi lama, nilainya disesuaikan
// utk tema accretion disk: background jadi biru-hitam sangat gelap
// (kedalaman ruang angkasa) drpd abu netral).
uint16_t BOOT_ACCENT = 0xFFFF; // putih -- teks utama
uint16_t BOOT_GLOW   = 0x39C7; // abu-kebiruan redup (dipertahankan, msh dipakai bootDrawParticleRing lama kalau mau dipanggil lg)
uint16_t BOOT_BG_TOP = 0x0841; // biru-hitam sangat gelap (kedalaman ruang angkasa)
uint16_t BOOT_BG_BOT = 0x0000; // hitam pekat

// Warna KHUSUS accretion disk: putih hangat (dalam) -> oranye (tengah)
// -> merah gelap (luar, meredup ke hitam).
uint16_t ACCR_DISK_IN  = 0xFFDD;
uint16_t ACCR_DISK_MID = 0xFD40;
uint16_t ACCR_DISK_OUT = 0x6000;

struct BootParticle{ float ang, r0, speed; };
#define BOOT_PARTICLE_N 8
BootParticle bootParticles[BOOT_PARTICLE_N];

void bootInitParticles(int w,int h){
  float maxR = sqrtf((float)(w*w+h*h))/2.0f;
  for(int i=0;i<BOOT_PARTICLE_N;i++){
    bootParticles[i].ang   = ((float)i/BOOT_PARTICLE_N)*2.0f*PI + (random(0,100)/100.0f)*0.5f;
    bootParticles[i].r0    = maxR*(0.75f + (random(0,100)/100.0f)*0.35f); // mulai dr pinggir/luar layar, radius sedikit acak per partikel
    bootParticles[i].speed = 0.85f + (random(0,100)/100.0f)*0.3f;        // variasi kecepatan spiral, biar gerakannya gak seragam kaku
  }
}
// "Cheap glow" utk 1 titik partikel -- inti kecil + 1 lingkaran buram
// lebih besar di belakangnya (warna di-lerp ke background), POLA SAMA
// PERSIS dgn halo logo di boot lama (bootStageRenPhone), cuma dipakai
// utk partikel individual di sini drpd 1 logo besar.
...
// Cincin accretion disk dgn gradasi radial MURAH: banyak drawCircle
// konsentris tipis dr tepi luar ke dalam, tiap lingkaran diwarnai hasil
// lerp sesuai jaraknya dr pusat (luar->tengah->dalam = gelap->oranye->
// putih) -- semangatnya SAMA PERSIS dgn bootDrawGradientBg yg linear,
// cuma versi radial, BUKAN alpha-blending sungguhan. Parameter "alpha"
// dipakai buat fade in/out cincin scr keseluruhan (0=nyaris nyatu ke
// background, 1=warna disk penuh) dgn cara nge-lerp hasil akhirnya SEKALI
// LAGI ke warna background -- trik yg sama dipakai fade judul di boot lama.
void bootDrawAccretionRing(int cx,int cy,float rOuter,float rInner,int rings,float alpha,uint16_t bg){
  if(rings<3) rings=3;
  for(int i=0;i<rings;i++){
    float t = (float)i/(rings-1); // 0=tepi luar, 1=tepi dalam
    float rad = rOuter - (rOuter-rInner)*t;
    uint16_t col = (t<0.5f)
      ? lerpColor565(ACCR_DISK_OUT, ACCR_DISK_MID, t*2.0f)
      : lerpColor565(ACCR_DISK_MID, ACCR_DISK_IN, (t-0.5f)*2.0f);
    col = lerpColor565(bg, col, alpha);
    canvas.drawCircle(cx,cy,(int)rad,col);
  }
}
...
// ---- Tahap 2: partikel muncul di pinggir, spiral makin cepat & makin
// rapat menuju pusat -- kurva percepatan pakai navEase (fungsi easing
// global yg sama dipakai transisi antar-app & Control Center, spy
// "rasa" gerakannya konsisten sama seluruh UI). ----
void bootStageAccretionParticles(int w,int h){
  int cx=w/2, cy=h/2-14; // sedikit di atas tengah, nyisain ruang utk teks+tagline di bawah (pola sama dgn boot lama)
  bootInitParticles(w,h);
  ...
  for(;;){
    ...
    for(int i=0;i<BOOT_PARTICLE_N;i++){
      ...
      float pp=p*pt.speed; if(pp>1.0f) pp=1.0f;
      float r = pt.r0*(1.0f-pp) + 6.0f*pp;   // menuju radius kecil dkt pusat (BUKAN nol persis, spy nyambung mulus ke cincin di tahap berikutnya)
      float ang = pt.ang + pp*pp*7.0f;        // spiral: pp*pp bikin rotasi makin cepat makin dkt pusat ("makin rapat")
      int px=cx+(int)(cosf(ang)*r), py=cy+(int)(sinf(ang)*r);
      int coreR=1+(int)(1.5f*pp);             // partikel sedikit membesar mendekati pusat (kesan "memanas")
      uint16_t col=lerpColor565(ACCR_DISK_MID,ACCR_DISK_IN,pp);
      bootDrawGlowDot(px,py,coreR,col,BOOT_BG_BOT);
    }
    push();
    if(lp>=1.0f) break;
    delay(8); // batas atas ~120fps -- durasi TOTAL ttp dikontrol millis() di atas, ini cuma jaga biar gak ngegas render
    if(wifiConnected && webServerRunning) webServer.handleClient();
    yield();
  }
}
// ---- Tahap 3: partikel menyatu jd cincin tipis bercahaya (accretion
// disk) di sekitar titik hitam kosong di tengah -- fade-in pakai trik
// cheap-glow radial (bootDrawAccretionRing), BUKAN partikel individual lg. ----
void bootStageAccretionRingForm(int w,int h){
  int cx=w/2, cy=h/2-14;
  const float R_OUT=44, R_IN=30;
  const unsigned long DUR=500;
  unsigned long t0=millis();
  for(;;){
    float lp=(float)(millis()-t0)/DUR; if(lp>1.0f) lp=1.0f;
    float p=navEase(lp);
    bootDrawGradientBg(0,0,w,h,h,BOOT_BG_TOP,BOOT_BG_BOT);
    bootDrawAccretionRing(cx,cy,R_OUT,R_IN,10,p,BOOT_BG_BOT); // alpha=p -> cincin fade-in dr background
    push();
    if(lp>=1.0f) break;
    delay(8);
    if(wifiConnected && webServerRunning) webServer.handleClient();
  ...
  delay(120); // jeda sesaat cincin penuh, sblm teks muncul (pola sama dgn jeda antar-tahap boot lama)
}
// ---- Tahap 4: teks "ACCRETION" fade-in dari titik pusat (scr warna,
// bukan skala -- font bawaan cuma bisa ukuran diskrit) dgn glow tipis
// di belakangnya, SEMENTARA cincin di sekitarnya memudar keluar. ----
void bootStageAccretionTitle(int w,int h){
  int cx=w/2, cy=h/2-14;
  const float R_OUT=44, R_IN=30;
  const char* title="ACCRETION";
  int tw=(int)strlen(title)*12; // textSize 2 -> ~12px/karakter
  const unsigned long DUR=700;
  unsigned long t0=millis();
  for(;;){
    float lp=(float)(millis()-t0)/DUR; if(lp>1.0f) lp=1.0f;
    float p=navEase(lp);
    ...
    bootDrawGradientBg(0,0,w,h,h,BOOT_BG_TOP,BOOT_BG_BOT);
    bootDrawAccretionRing(cx,cy,R_OUT,R_IN,10,1.0f-p,BOOT_BG_BOT); // cincin: alpha turun dr 1->0 (memudar)

    canvas.setTextSize(2);
    canvas.setTextColor(lerpColor565(BOOT_BG_BOT,ACCR_DISK_MID,p*0.6f)); // glow tipis oranye redup di belakang (offset 1px)
    canvas.setCursor(cx-tw/2-1,cy-8-1); canvas.print(title);
    canvas.setTextColor(lerpColor565(BOOT_BG_BOT,0xFFFF,p));            // teks utama putih, fade dr hitam
    canvas.setCursor(cx-tw/2,cy-8); canvas.print(title);
...
// ---- Tahap 5: "ACCRETION" slide+fade keluar (naik dikit sambil
// memudar), "NyxOS" muncul di bawah posisi teks sebelumnya -- lebih
// kecil & polos, SENGAJA tanpa efek cincin/glow apapun. ----
void bootStageAccretionToOS(int w,int h){
  int cx=w/2, cy=h/2-14;
  const char* title="ACCRETION"; int twTitle=(int)strlen(title)*12;
  const char* osName="NyxOS";    int twOs=(int)strlen(osName)*6; // textSize 1 -> ~6px/karakter
  const unsigned long DUR=500;
  unsigned long t0=millis();
  for(;;){
    float lp=(float)(millis()-t0)/DUR; if(lp>1.0f) lp=1.0f;
    ...
    float slideUp=14.0f*p;
    canvas.setTextSize(2);
    canvas.setTextColor(lerpColor565(0xFFFF,BOOT_BG_BOT,p));
    canvas.setCursor(cx-twTitle/2,(int)(cy-8-slideUp)); canvas.print(title);

    canvas.setTextSize(1); // minimalis: lebih kecil, gak ada glow/cincin spt "ACCRETION"
    canvas.setTextColor(lerpColor565(BOOT_BG_BOT,0xFFFF,p));
    canvas.setCursor(cx-twOs/2,cy+24); canvas.print(osName);
...
void bootStageAccretionTagline(int w,int h){
  int cx=w/2, cy=h/2-14;
  const char* osName="NyxOS"; int twOs=(int)strlen(osName)*6;
  ...
  for(;;){
    ...
    bootDrawGradientBg(0,0,w,h,h,BOOT_BG_TOP,BOOT_BG_BOT);
    canvas.setTextSize(1); canvas.setTextColor(0xFFFF);
    canvas.setCursor(cx-twOs/2,cy+24); canvas.print(osName);
    canvas.setTextColor(lerpColor565(BOOT_BG_BOT,0x8C51,p)); // abu redup -- tone yg sama dgn tagline boot lama
    canvas.setCursor(cx-twTag/2,cy+42); canvas.print(tag);
...
void runBootSequence(){
  ...
  bootFade(0,255,8,6); // fade masuk gelap->terang (reuse persis fungsi boot lama)

  bootStageAccretionPause(w,h);       // 1) jeda hitam (0.3s)
  bootStageAccretionParticles(w,h);   // 2) partikel spiral menuju pusat (1.0s)
  bootStageAccretionRingForm(w,h);    // 3) partikel menyatu jd cincin accretion disk (0.5s)
  bootStageAccretionTitle(w,h);       // 4) "ACCRETION" fade-in, cincin memudar (0.7s)
  bootStageAccretionToOS(w,h);        // 5) "ACCRETION" keluar, "NyxOS" muncul (0.5s)
  bootStageAccretionTagline(w,h);     // 6) tagline fade-in (0.5s)
