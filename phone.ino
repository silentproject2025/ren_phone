...
void handleWebRoot() {
  ...
  html += "<div class='card'><h3>Upload Lagu ke PSRAM (bukan SD Card)</h3>";
  html += "<p style='color:#aaa;font-size:12px;'>WAV mono 16-bit PCM saja, maks "+String(SONG_PSRAM_MAX_BYTES/1024/1024)+"MB (&asymp;1-2 menit audio). Hilang saat restart. Convert MP3 dulu: <code>ffmpeg -i lagu.mp3 -ac 1 -ar 22050 -acodec pcm_s16le lagu.wav</code></p>";
  if(songPsramBuf && songDataLen>0){
    html += "<p>Tersimpan: <b>"+songPsramName+"</b> ("+String(songDataLen/1024)+" KB, "+String(songSampleRate)+"Hz, "+String(songChannels)+"ch)</p>";
  } else if(songUploadError.length()>0){
    html += "<p style='color:#ff5252;'>Gagal: "+songUploadError+"</p>";
  } else {
    html += "<p style='color:#777;'>Belum ada lagu di PSRAM.</p>";
  }
  html += "<form method='POST' action='/upload_song' enctype='multipart/form-data'>";
  html += "<input type='file' name='upload' accept='.wav' required style='margin-bottom:10px;'><br>";
  html += "<input type='submit' class='btn' value='Upload ke PSRAM'>";
  html += "</form><p style='color:#777;font-size:12px;margin-top:8px;'>Puter/stop-nya lewat app MP3 Player di layar HP.</p>";
  html += "</div>";
...
void startWebServer(){
  ...
  // v35: upload lagu (WAV) LANGSUNG ke buffer PSRAM, terpisah dr /upload
  // biasa (yg nulis ke SD Card) -- lihat blok "LAGU DI PSRAM" dkt AVI Player.
  webServer.on("/upload_song", HTTP_POST, [](){
    webServer.sendHeader("Location", "/");
    webServer.send(303);
  }, handleSongUploadStream);
...
// =================================================================
// v35: MODE "NGOBROL" (live voice chat) -- ketuk Mic sekali, INMP441
// rekam -> auto-transkrip (fungsi transkrip yg SUDAH ADA, micTranscribeAndFill())
// -> kalau mode ini nyala, transkrip OTOMATIS dikirim ke Gemini (bukan cuma
// diisi ke kolom prompt spt v27) -> begitu jawaban teks Gemini siap,
// OTOMATIS diucapkan lewat speaker MAX98357A (lihat blok "AI SPEAK (TTS)"
// dkt app MP3 Player, krn numpang objek mp3Audio yg sama) -> selesai
// bicara, mic OTOMATIS rekam lagi -> mirip ngobrol beneran, gak perlu
// ketuk2 lagi tiap giliran. Ketuk tombol "Ngobrol" lagi (atau tombol Mic
// pas lg ngomong) kapan aja utk keluar dari mode ini.
// =================================================================
bool aiLiveChatMode = false;
// Diisi di ujung sendGeminiRequest() (jalan di geminiTaskFunc, task
// TERPISAH) -- SENGAJA cuma nitip teks & flag di sini, BUKAN langsung
// manggil fungsi bicara dari dalam task tsb (mp3Audio dipakai jg dari
// loop() utama, jd biar aman dari race condition semua pemakaian
// mp3Audio DISIPLIN cuma dari context loop()/touch handler -- sama pola
// spt aiResponse/aiLoading yg jg cuma "dititip" dari task lalu diproses
// belakangan oleh loop()/renderer).
volatile bool   aiSpeakPending = false;
String          aiSpeakPendingText = "";
...
void sendGeminiRequest(String promptText) {
  ...
  // v35: mode Ngobrol -- titip jawaban ke antrean TTS (dieksekusi dr loop(),
  // lihat catatan di deklarasi aiSpeakPending soal kenapa gak langsung di
  // sini). Kalau gagal (!ok) gak usah diucapkan -- isinya cuma pesan error.
  if(ok && aiLiveChatMode && resp.length()>0){
    aiSpeakPendingText = resp;
    aiSpeakPending = true;
  }
  aiLoading = false;
  aiRequestStartMillis = 0;
...
// APP: AI CHAT
// =============================================
#define AI_CHAT_TOP 64 // v35: dinaikkan dr 46 -- kasih ruang baris ke-2 (toggle mode Ngobrol) di header
...
void drawAiChat(LGFX_Sprite& s){
  ...
  // v35: baris ke-2 header -- toggle mode "Ngobrol" (live voice chat,
  // lihat blok "MODE NGOBROL" dkt deklarasi aiPrompt/aiResponse) +
  // status singkat selagi mode ini aktif.
  int liveBtnY=44, liveBtnH=16;
  s.fillRoundRect(6,liveBtnY,90,liveBtnH,4, aiLiveChatMode?T().good:T().surface2);
  s.setTextColor(aiLiveChatMode?T().bg:T().subtext); s.setCursor(12,liveBtnY+4);
  s.print(aiLiveChatMode? "Ngobrol: ON" : "Ngobrol: OFF");
  if(aiLiveChatMode){
    s.setTextColor(T().subtext); s.setCursor(102,liveBtnY+4);
    if(aiSpeaking) s.print("AI sedang bicara...");
    else if(micRecording) s.print("Dengerin kamu...");
    else if(micTranscribing) s.print("Mikir...");
    else if(aiLoading) s.print("Nunggu Gemini...");
    else s.print("Ketuk Mic utk mulai");
  }
...
void aiTouch(int x,int y,bool held,bool isNew){
  ...
  if(isNew){
    ...
    if(x>=micBtnX && x<=micBtnX+micBtnW && y>=24 && y<=42){
      if(aiSpeaking) aiSpeakStop();      // v35: ketuk Mic selagi AI lg bicara -> potong, giliran user
      else if(micRecording) micStopRecording();
      else if(!micTranscribing) micStartRecording();
      return;
    }

    // v35: tombol toggle mode Ngobrol (live voice chat), baris ke-2 header.
    if(x>=6 && x<=96 && y>=44 && y<=60){
      aiLiveChatMode = !aiLiveChatMode;
      if(!aiLiveChatMode){
        // matiin mode -> hentikan siklus otomatis yg lg jalan (rekam/bicara)
        if(micRecording) micStopRecording();
        if(aiSpeaking) aiSpeakStop();
        showToast("Mode Ngobrol dimatikan");
      } else {
        showToast("Mode Ngobrol aktif -- ketuk Mic utk mulai");
      }
      needRedraw=true;
      return;
    }
...
// =============================================
// AI SPEAK (TTS) -- BARU v35, bagian dari mode "Ngobrol" (lihat blok
// "MODE NGOBROL" dkt deklarasi aiPrompt/aiResponse).
// -----------------------------------------------------------------
// Ucapkan jawaban teks Gemini lewat speaker MAX98357A. SENGAJA numpang
// objek `mp3Audio` yg SAMA persis dgn MP3 Player (bukan bikin objek Audio
// baru) -- alasannya MURNI RAM: objek Audio itu sendiri berat (6 decoder
// sekaligus, lihat catatan "v27 FIX (RAM internal)" di atas mp3EnsureAudio()),
// jd numpang 1 objek yg sama drpd bikin objek kedua yg mubazir motong RAM
// internal dua kali lipat.
//
// Library ESP32-audioI2S yg sudah ke-include (Audio.h) TERNYATA sudah ada
// dukungan bawaan connecttospeech(text, lang) yg manggil Google Translate
// TTS (gratis, GAK BUTUH API key terpisah dr Gemini) -- makanya dipakai di
// sini drpd bikin ulang jalur TTS custom lewat Gemini (yg base64 PCM-nya
// jauh lebih ribet & belum tentu didukung model gratis yg dipakai project
// ini). Endpoint TTS gratis ini ada batas panjang teks per request, jd
// jawaban Gemini (bisa ratusan karakter) DIPOTONG dulu jadi potongan2
// pendek (aiSpeakSplit()) & diputar BERURUTAN, potongan berikutnya baru
// jalan setelah potongan sebelumnya beneran selesai (dicek isRunning()).
//
// CATATAN JUJUR: TTS gratis begini kadang lambat/gak stabil (server pihak
// ke-3, bukan punya Google Cloud TTS resmi) -- kalau sering gagal/putus,
// itu wajar utk layanan gratis begini, bukan berarti kodenya rusak.
// =============================================
#define AI_SPEAK_LANG "id"        // kode bahasa Google Translate TTS ("id"=Indonesia). Ganti ke "en" dll kalau mau.
#define AI_SPEAK_CHUNK_CHARS 170  // batas aman per potongan (endpoint TTS gratis ini biasanya mentok di sekitar 200 char)
#define AI_SPEAK_MAX_CHUNKS 20    // jaring pengaman -- jawaban Gemini yg KELEWAT panjang dipotong (drpd nyangkut lama/ngerusak antrean)
#define AI_SPEAK_START_GRACE_MS 500 // jeda sblm mulai ngecek isRunning()==false, cegah "keburu dianggap selesai" pas baru mulai connect

String   aiSpeakChunks[AI_SPEAK_MAX_CHUNKS];
int      aiSpeakChunkCount = 0;
int      aiSpeakChunkIdx   = -1;
bool     aiSpeaking = false;
unsigned long aiSpeakChunkStartMs = 0;

// Potong `text` jadi beberapa kalimat pendek (<=AI_SPEAK_CHUNK_CHARS),
// diusahakan motong di akhir kalimat (. ? ! atau newline) atau spasi
// terdekat SEBELUM batas -- bukan motong sembarang di tengah kata.
int aiSpeakSplit(const String& text, String* outChunks, int maxChunks){
  int count=0;
  int len=(int)text.length();
  int pos=0;
  while(pos<len && count<maxChunks){
    int remain = len-pos;
    if(remain<=AI_SPEAK_CHUNK_CHARS){
      String piece = text.substring(pos); piece.trim();
      if(piece.length()>0){ outChunks[count++]=piece; }
      break;
    }
    int cut=-1;
    // cari titik potong "alami" (akhir kalimat) di dalam jendela batas
    for(int i=pos+AI_SPEAK_CHUNK_CHARS; i>pos+20; i--){
      char c=text.charAt(i);
      if(c=='.'||c=='?'||c=='!'||c=='\n'){ cut=i+1; break; }
    }
    if(cut<0){ // gak ketemu akhir kalimat -> potong di spasi terdekat aja, drpd motong tengah kata
      for(int i=pos+AI_SPEAK_CHUNK_CHARS; i>pos+20; i--){
        if(text.charAt(i)==' '){ cut=i+1; break; }
      }
    }
    if(cut<0) cut = pos+AI_SPEAK_CHUNK_CHARS; // jaring pengaman terakhir: potong paksa
    String piece = text.substring(pos,cut); piece.trim();
    if(piece.length()>0){ outChunks[count++]=piece; }
    pos=cut;
  }
  return count;
...
// Mulai ucapkan `text`. Motong dulu jalur audio lain yg mungkin lg jalan
// (lagu SD via mp3Audio, video AVI via aviI2S) krn cuma ada 1 modul
// MAX98357A fisik -- SAMA persis disiplinnya spt mp3Play()/aviAppEnter().
void aiSpeakStart(const String& text){
  if(text.length()==0) return;
  if(WiFi.status()!=WL_CONNECTED){ showToast("WiFi belum terhubung, gak bisa TTS"); return; }
  mp3StopAll();
  if(aviI2SReady) aviReleaseI2S(); // jaga2 kalau lg dipegang song-PSRAM/AVI player
  songPsramStop(); // v35: forward-declared di bawah -- hentikan playback lagu PSRAM kalau lg jalan
  mp3EnsureAudio();
  aiSpeakChunkCount = aiSpeakSplit(text, aiSpeakChunks, AI_SPEAK_MAX_CHUNKS);
  if(aiSpeakChunkCount==0) return;
  aiSpeakChunkIdx = 0;
  aiSpeaking = true;
  aiSpeakChunkStartMs = millis();
  mp3Audio->connecttospeech(aiSpeakChunks[0].c_str(), AI_SPEAK_LANG);
}

// Hentikan paksa TTS yg lg jalan (dipanggil kalau user ketuk tombol
// Mic/Ngobrol lg selagi AI msh bicara).
void aiSpeakStop(){
...
// Dipanggil TIAP loop() (pola sama spt mp3Audio->loop()) -- majuin ke
// potongan kalimat berikutnya begitu potongan skrg beneran selesai
// diputar. Kalau semua potongan abis & mode Ngobrol msh nyala, OTOMATIS
// mulai rekam giliran user berikutnya (siklus "ngobrol").
void aiSpeakStep(){
  if(!aiSpeaking) return;
  if(millis()-aiSpeakChunkStartMs < AI_SPEAK_START_GRACE_MS) return; // kasih waktu connect dulu
  if(mp3AudioInited && mp3Audio->isRunning()) return; // msh muter potongan skrg
  aiSpeakChunkIdx++;
  if(aiSpeakChunkIdx < aiSpeakChunkCount){
    aiSpeakChunkStartMs = millis();
    mp3Audio->connecttospeech(aiSpeakChunks[aiSpeakChunkIdx].c_str(), AI_SPEAK_LANG);
    return;
  }
  // semua potongan selesai diucapkan
  aiSpeaking=false;
  aiSpeakChunkIdx=-1;
  if(aiLiveChatMode && !micRecording && !micTranscribing){
    micStartRecording(); // lanjut giliran user -- inilah yg bikin kerasa "ngobrol"
  }
}
...
// v35: diturunin dr 5 ke 2 -- ruang buat daftar lagu SD menyusut krn
// nambah baris status "Lagu PSRAM" + indikator halaman di atasnya (lihat
// songRowY di drawMp3App()). Dihitung supaya PAS (rowH gak kejegal floor
// 28 di mp3RowH()), drpd baris list numpuk ketutup tombol kontrol di bawah.
#define MP3_PER_PAGE 2
int mp3ListTop(){ return STATUS_H+90; } // v35: +34 dr semula (STATUS_H+56), kasih ruang baris status lagu PSRAM + indikator halaman
...
void drawMp3App(LGFX_Sprite& s){
  ...
  // v35: baris status lagu PSRAM (upload via web server "/", lihat blok
  // "LAGU DI PSRAM" dkt AVI Player) -- terpisah dr "now playing" SD di
  // atas krn sumbernya independen, walau gantian pakai speaker fisik yg sama.
  int songRowY = STATUS_H+52;
  s.fillRoundRect(6,songRowY,SCR_W-12,20,6,T().surface2);
  s.setTextColor(T().subtext); s.setTextSize(1); s.setCursor(10,songRowY+6);
  if(songPsramBuf && songDataLen>0){
    String nm=songPsramName; if((int)nm.length()>20) nm=nm.substring(0,20);
    s.print((songPlaying? "[PSRAM,Main] ":"[PSRAM] ")+nm);
    s.fillRoundRect(SCR_W-60,songRowY+1,54,18,4, songPlaying?T().danger:T().good);
    s.setTextColor(0xFFFF); s.setCursor(SCR_W-52,songRowY+5);
    s.print(songPlaying? "Stop":"Main");
  } else {
    s.print("PSRAM: blm ada lagu (upload lwt web)");
  }
...
void mp3AppTouch(int x,int y,bool held,bool isNew){
  ...
  // v35: tombol Main/Stop lagu PSRAM (koordinat SAMA dgn yg digambar di
  // drawMp3App() -- lihat songRowY di sana).
  int songRowY = STATUS_H+52;
  if(songPsramBuf && songDataLen>0 && x>=SCR_W-60 && x<=SCR_W-6 && y>=songRowY+1 && y<=songRowY+19){
    if(songPlaying) songPsramStop(); else songPsramPlay();
    needRedraw=true;
    return;
  }
...
// =============================================
// LAGU DI PSRAM -- BARU v35. Upload 1 file lagu lewat Web File Manager
// (halaman "/", form terpisah dr form upload SD biasa) LANGSUNG ke
// buffer PSRAM (BUKAN ditulis ke SD Card) -- begitu ESP32 restart atau
// upload baru masuk, lagu lama di buffer ini HILANG (ini RAM, bukan
// penyimpanan permanen -- kalau mau permanen ya tetap pakai folder
// /mp3 di SD Card spt MP3 Player biasa di atas).
// -----------------------------------------------------------------
// CATATAN JUJUR soal format file: MP3/AAC/dll itu data TERKOMPRESI --
// buat mendekodenya dari buffer memori mentah (bukan dari sebuah file
// beneran di SD/SPIFFS) library ESP32-audioI2S yg dipakai project ini
// (Audio.h) TIDAK terbukti/terdokumentasi resmi punya jalur "putar
// langsung dari byte array di RAM" (yg ada cuma connecttoFS/
// connecttohost/connecttospeech, semuanya minta sebuah fs::FS atau URL,
// bukan pointer buffer). Drpd nekat nulis workaround yg BELUM TENTU
// beneran jalan di hardware (risikonya malah bikin device nge-hang pas
// decode), fitur ini SENGAJA dibikin nerima format WAV PCM MENTAH (gak
// terkompresi) & diputar LANGSUNG ke I2S -- teknik yg SAMA PERSIS & UDAH
// TERBUKTI JALAN spt audio AVI Player di atas (aviI2S, PCM ditulis
// langsung ke I2S tanpa decoder tambahan sama sekali).
//
// Konsekuensinya: krn WAV gak terkompresi, "beberapa MB" cuma cukup utk
// SEKITAR 1-2 MENIT audio (bukan 1 lagu utuh 3-4 menit) -- ini batasan
// FORMAT/fisika data, BUKAN batasan kode. Kalau file aslinya MP3, convert
// dulu pakai ffmpeg (pola perintahnya SAMA gayanya dgn yg dipakai fitur
// AVI Player di atas):
//   ffmpeg -i lagu.mp3 -ac 1 -ar 22050 -acodec pcm_s16le lagu.wav
// (mono 22050Hz 16-bit -> sekitar 2 menit muat di cap 5MB default di
// bawah -- naikkan SONG_PSRAM_MAX_BYTES kalau PSRAM 8MB masih longgar,
// cek dulu di app HWmonitor).
// =============================================
#define SONG_PSRAM_MAX_BYTES (5UL*1024*1024) // cap default 5MB

uint8_t* songPsramBuf = nullptr;   // buffer mentah hasil upload (isinya file WAV APA ADANYA, termasuk headernya)
size_t   songPsramCap = 0;         // kapasitas buffer yg dialokasikan
size_t   songPsramLen = 0;         // jumlah byte yg beneran kepakai (ukuran file WAV hasil upload)
String   songPsramName = "";       // nama file asli, cuma buat ditampilkan
uint32_t songDataOffset = 0;       // offset byte awal chunk "data" (audio mentah) hasil parse header
uint32_t songDataLen = 0;          // panjang chunk "data" dlm byte
uint32_t songSampleRate = 22050;
uint16_t songChannels = 1;
uint16_t songBitsPerSample = 16;
bool     songUploading = false;
size_t   songUploadWritten = 0;
String   songUploadError = "";     // pesan error terakhir (mis. "Bukan WAV valid"), ditampilin di halaman web & app MP3 Player

volatile bool songPlaying = false;
size_t   songPlayOffset = 0;

uint32_t songRdU32(const uint8_t* b, size_t p){
  return (uint32_t)b[p] | ((uint32_t)b[p+1]<<8) | ((uint32_t)b[p+2]<<16) | ((uint32_t)b[p+3]<<24);
}
uint16_t songRdU16(const uint8_t* b, size_t p){
  return (uint16_t)b[p] | ((uint16_t)b[p+1]<<8);
}

// Parser chunk WAV/RIFF manual (jalan per-chunk, BUKAN cuma asumsi header
// 44-byte tetap -- beberapa WAV hasil export py chunk tambahan spt
// "LIST"/"fact" sblm "data"). Mengisi songDataOffset/songDataLen/
// songSampleRate/songChannels/songBitsPerSample kalau berhasil.
bool songParseWavHeader(){
  songUploadError = "";
  if(songPsramLen < 12 || memcmp(songPsramBuf,"RIFF",4)!=0 || memcmp(songPsramBuf+8,"WAVE",4)!=0){
    songUploadError = "Bukan file WAV valid (header RIFF/WAVE tidak ketemu)";
    return false;
  }
  size_t pos=12;
  bool foundFmt=false, foundData=false;
  while(pos+8 <= songPsramLen){
    char id[5]; memcpy(id, songPsramBuf+pos, 4); id[4]=0;
    uint32_t sz = songRdU32(songPsramBuf, pos+4);
    size_t bodyStart = pos+8;
    if(strcmp(id,"fmt ")==0 && bodyStart+16<=songPsramLen){
      songChannels      = songRdU16(songPsramBuf, bodyStart+2);
      songSampleRate     = songRdU32(songPsramBuf, bodyStart+4);
      songBitsPerSample  = songRdU16(songPsramBuf, bodyStart+14);
      foundFmt=true;
    } else if(strcmp(id,"data")==0){
      songDataOffset = (uint32_t)bodyStart;
      uint32_t avail = (uint32_t)(songPsramLen-bodyStart);
      songDataLen    = sz<avail ? sz : avail;
      foundData=true;
      break; // audio mentah biasanya chunk terakhir -- gak perlu lanjut parse
    }
    pos = bodyStart + sz + (sz&1); // chunk WAV selalu rata genap (padding 1 byte kalau sz ganjil)
  }
  if(!foundFmt || !foundData){
    songUploadError = "Header WAV gak lengkap (chunk fmt/data gak ketemu)";
    return false;
  }
  if(songBitsPerSample!=16){
    songUploadError = "Cuma dukung WAV 16-bit PCM (file ini "+String(songBitsPerSample)+"-bit)";
    return false;
  }
  if(songChannels<1 || songChannels>2){
    songUploadError = "Jumlah channel WAV gak didukung ("+String(songChannels)+")";
    return false;
  }
  return true;
}
...
void songPsramFree(){
  if(songPsramBuf){ heap_caps_free(songPsramBuf); songPsramBuf=nullptr; }
  songPsramCap=0; songPsramLen=0; songDataOffset=0; songDataLen=0; songPsramName="";
}

void songPsramStop(){
  if(!songPlaying) return;
  songPlaying=false;
  aviReleaseI2S(); // lepas I2S biar app lain (MP3/AVI/TTS Ngobrol) bisa gantian pakai
}

void songPsramPlay(){
  if(!songPsramBuf || songDataLen==0){ showToast("Belum ada lagu di PSRAM"); vibWarn(); return; }
  mp3StopAll();   // matikan MP3 Player SD kalau lg jalan (rebutan 1 modul speaker fisik yg sama)
  aiSpeakStop();  // matikan TTS AI kalau lg ngomong
  if(aviI2SReady) aviReleaseI2S();
  aviAudioSampleRate = songSampleRate; // numpang I2SClass & fungsi ensure/release AVI Player -- pola PCM->I2S-nya SAMA PERSIS
  aviAudioChannels   = songChannels;
  aviEnsureI2S();
  songPlayOffset = songDataOffset;
  songPlaying = true;
  vibSuccess();
}

// Dipanggil TIAP loop() (pola sama spt mp3Audio->loop()) -- nulis
// SEPOTONG PCM ke I2S tiap kali dipanggil (BUKAN sekaligus semua file,
// biar UI/touchscreen gak macet total selama lagu diputar). aviI2S.write()
// sendiri blocking sesaat (nunggu buffer DMA I2S kosong) -- itu yg
// otomatis jaga tempo playback real-time, SAMA PERSIS spt dijelaskan di
// catatan AVI Player.
#define SONG_STEP_BYTES 2048
void songPsramStep(){
  if(!songPlaying) return;
  uint32_t end = songDataOffset+songDataLen;
  if(songPlayOffset >= end){ songPsramStop(); needRedrawNow(); return; }
  size_t remain = end - songPlayOffset;
  size_t n = remain<SONG_STEP_BYTES ? remain : SONG_STEP_BYTES;
  aviI2S.write(songPsramBuf+songPlayOffset, n);
  songPlayOffset += n;
}

// ---- Web upload handler (dipasang ke rute POST /upload_song di
// startWebServer(), lihat blok WEB SERVER di awal file) ----
void handleSongUploadStream(){
  HTTPUpload& upload = webServer.upload();
  if(upload.status==UPLOAD_FILE_START){
    songPsramStop();
    songPsramFree();
    songUploadError="";
    songPsramBuf = (uint8_t*)heap_caps_malloc(SONG_PSRAM_MAX_BYTES, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if(!songPsramBuf){
      songUploadError = "RAM PSRAM tidak cukup utk buffer lagu";
      songUploading=false;
      return;
    }
    songPsramCap = SONG_PSRAM_MAX_BYTES;
    songUploadWritten = 0;
    songUploading = true;
    songPsramName = upload.filename;
  } else if(upload.status==UPLOAD_FILE_WRITE){
    if(!songUploading || !songPsramBuf) return;
    size_t room = songPsramCap - songUploadWritten;
    size_t toCopy = upload.currentSize < room ? upload.currentSize : room;
    if(toCopy>0){
      memcpy(songPsramBuf+songUploadWritten, upload.buf, toCopy);
      songUploadWritten += toCopy;
    }
    if(toCopy < upload.currentSize){
      // Lebih besar dr cap -> batalkan, drpd nyimpen file WAV yg kepotong/rusak
      songUploadError = "File lebih besar dari batas "+String(SONG_PSRAM_MAX_BYTES/1024/1024)+"MB, upload dibatalkan";
      songUploading=false;
      songPsramFree();
    }
  } else if(upload.status==UPLOAD_FILE_END){
    if(songUploading && songPsramBuf){
      songPsramLen = songUploadWritten;
      songUploading=false;
      if(!songParseWavHeader()){
        songPsramFree(); // header invalid -- jangan nyimpen sampah di PSRAM
      }
      needRedrawNow();
    }
...
void micTranscribeAndFill(uint8_t* wavBuf, size_t wavLen){
  ...
  aiPrompt = transcript;
  vibSuccess();
  // v35: mode Ngobrol -- lanjut OTOMATIS ke Gemini, gak nunggu user ketuk
  // Kirim manual (persis kayak dulu). Kalau mode ini OFF, perilaku LAMA
  // (v27) tetep jalan apa adanya: cuma isi kolom prompt, nunggu ditinjau &
  // dikirim manual oleh user.
  if(aiLiveChatMode){
    showToast("Terdengar: \"" + (transcript.length()>40? transcript.substring(0,40)+"..":transcript) + "\"");
    needRedrawNow();
    triggerGeminiAI();
  } else {
...
void loop(){
  ...
  // v35: majuin TTS "Ngobrol" (potong kalimat berikutnya begitu potongan
  // skrg selesai) & lagu PSRAM (nulis potongan PCM berikutnya ke I2S) --
  // dua2nya pola non-blocking sama spt mp3Audio->loop() di atas.
  aiSpeakStep();
  songPsramStep();
  // v35: jawaban Gemini siap & mode Ngobrol aktif -> mulai TTS di sini
  // (context loop() utama, BUKAN dari dalam geminiTaskFunc -- lihat
  // catatan panjang di deklarasi aiSpeakPending soal kenapa).
  if(aiSpeakPending){
    aiSpeakPending=false;
    aiSpeakStart(aiSpeakPendingText);
  }
  // v35 RAM internal: objek mp3Audio (dipakai bareng MP3 Player & TTS
  // Ngobrol) BERAT drpd sisi RAM internal (6 decoder sekaligus, lihat
  // catatan "v27 FIX" di atas mp3EnsureAudio()) -- kalau bener2 nganggur
  // lumayan lama (gak ada lagu SD, gak ada TTS ngomong, gak ada lagu
  // PSRAM), lepas objeknya & kembalikan RAM internal itu ke pool umum,
  // spt Gemini/fitur lain yg butuh largestBlock besar. Objeknya otomatis
  // dibikin ulang (mp3EnsureAudio()) begitu dibutuhkan lagi -- SENGAJA
  // timeout-nya lumayan panjang (3 menit) drpd tiap giliran Ngobrol,
  // supaya gak bolak-balik alloc/delete tiap kalimat (bisa bikin heap
  // internal terfragmentasi kalau terlalu sering).
  static unsigned long mp3LastActiveMs = 0;
  if(mp3AudioInited){
    bool mp3ActiveNow = mp3Audio->isRunning() || aiSpeaking || songPlaying;
    if(mp3ActiveNow || mp3LastActiveMs==0) mp3LastActiveMs = nowMs; // refresh baseline selagi aktif, ATAU inisialisasi baseline pertama kali
    if(!mp3ActiveNow && (nowMs-mp3LastActiveMs)>180000UL){
      delete mp3Audio; mp3Audio=nullptr; mp3AudioInited=false;
      mp3LastActiveMs=0;
      Serial.println("[RAM] mp3Audio dilepas (idle 3 menit) - RAM internal direklaim");
    }
  } else {
    mp3LastActiveMs = 0;
  }
