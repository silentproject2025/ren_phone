...
void bootStageRenPhone(int w,int h){
  ...
  // fase 0: kedipan noise/static sekilas sblm logo muncul, kesan "sinyal masuk"
  for(int f=0; f<6; f++){
    canvas.fillSprite(0x0000);
    glitchNoiseBlocks(w,h,70);
    push();
    delay(26);
  }

  // fase 1: logo membesar dgn distorsi glitch acak (posisi loncat, RGB split, scan tear)
  for(int f=0; f<=22; f++){
    float p=f/22.0f;
    canvas.fillSprite(0x0000);
    bootDrawParticleRing(cx,cy,60+(int)(10*p),10,0x2965,p*4.0f);
    int boxSize=(int)(20+46*p);
    bool glitchFrame = (random(0,10)<3);
    int jx = glitchFrame? random(-6,6):0;
    int jy = glitchFrame? random(-3,3):0;
    canvas.fillRoundRect(cx-boxSize/2+jx,cy-boxSize/2+jy,boxSize,boxSize,boxSize/4, glitchFrame?0x07FF:accent);
    canvas.setTextColor(0x0000); canvas.setTextSize(3);
    canvas.setCursor(cx-9+jx,cy-12+jy); canvas.print("R");
    if(p>0.4f) glitchTextRGB(cx,cy+50,"REN PHONE",2);
    if(glitchFrame) glitchScanTear(w,h,3);
    push();
    delay(16);
  }
  // beberapa kedipan glitch terakhir sblm gambar stabil
  for(int i=0;i<4;i++){
    canvas.fillSprite(0x0000);
    bootDrawParticleRing(cx,cy,70,10,0x2965,i*0.7f);
    canvas.fillRoundRect(cx-33,cy-33,66,66,16,accent);
    canvas.setTextColor(0x0000); canvas.setTextSize(3);
    canvas.setCursor(cx-9,cy-12); canvas.print("R");
    glitchTextRGB(cx,cy+50,"REN PHONE",2);
    if(i%2==0) glitchScanTear(w,h,2);
    push();
    delay(60);
  }
  // frame final: stabil bersih, tagline muncul
  canvas.fillSprite(0x0000);
  bootDrawParticleRing(cx,cy,70,10,0x2965,0);
  canvas.fillRoundRect(cx-33,cy-33,66,66,16,accent);
  canvas.setTextColor(0x0000); canvas.setTextSize(3);
  canvas.setCursor(cx-9,cy-12); canvas.print("R");
...
void bootStageSanzXOS(int w,int h){
  ...
  // fase 1: hexagon muncul dgn distorsi glitch (jitter sudut, scan tear)
  for(int f=0; f<=20; f++){
    float p=f/20.0f;
    canvas.fillSprite(0x0000);
    float r=34*p;
    ...
    for(int i=0;i<6;i++){
      ...
      int jitter = glitchFrame? random(-4,4):0;
      hx[i]=cx+(int)(cosf(ang)*r)+jitter; hy[i]=cy+(int)(sinf(ang)*r)+jitter;
    }
    for(int i=0;i<6;i++) canvas.drawLine(hx[i],hy[i],hx[(i+1)%6],hy[(i+1)%6], glitchFrame?0xF81F:hexColor);
    canvas.setTextColor(hexColor); canvas.setTextSize(2);
    canvas.setCursor(cx-8,cy-8); canvas.print("S");
    if(p>0.4f) glitchTextRGB(cx,cy+48,"SanzX OS",2);
    if(glitchFrame) glitchScanTear(w,h,2);
    push();
    delay(16);
  }

  // fase 2: "terminal boot log" ala hacker, karakter kadang corrupt sekilas
  const char* lines[5] = {
    "> mounting /dev/sanzx0 ... OK",
    "> loading kernel modules ... OK",
    "> init display driver ... OK",
    "> calibrating reality.exe ... OK",
    "> sanzx OS ready_"
  };
  canvas.fillSprite(0x0000);
  int ty=h/2-40;
  ...
  for(int li=0; li<5; li++){
    ...
    for(int c=0;c<=(int)full.length();c+=2){
      canvas.fillRect(8,ty+li*13,w-16,10,0x0000);
      canvas.setTextColor(0x07E0); // hijau ala terminal
      canvas.setCursor(8,ty+li*13);
      String shown = full.substring(0,c);
      if(c<(int)full.length() && random(0,4)==0){
        char corrupt[2]={(char)random(33,126),0};
        canvas.print(shown+corrupt);
      } else {
        canvas.print(shown);
      }
      push();
  ...
  // fase 3: hex logo balik dgn titik loading + kedipan RGB split terakhir
  for(int loop2=0; loop2<2; loop2++){
    for(int i=0;i<3;i++){
      ...
      canvas.setTextColor(hexColor); canvas.setTextSize(2);
      canvas.setCursor(cx-8,cy-8); canvas.print("S");
      glitchTextRGB(cx,cy+48,"SanzX OS",2);
      for(int d=0; d<3; d++){
