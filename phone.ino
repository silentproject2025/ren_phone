...
// (setup & kalibrasi touch dibalikin PERSIS ke logic kode lama v6
// yang sudah terbukti tidak pernah kena bug ini)
...
//  1. AKAR MASALAH: di v8 (dan versi2 sesudahnya yang portrait-default),
//     urutan di setup() itu display.setRotation(0) (paksa PORTRAIT) ->
//     KALIBRASI TOUCH dijalankan di rotasi portrait itu -> baru
//     BELAKANGAN loadOrientPref()+applyOrientation() menerapkan
//     orientasi yang SEBENARNYA tersimpan di NVS. Kalau nilai
//     tersimpan itu ternyata beda dari rotasi saat kalibrasi (mis.
//     LANDSCAPE), maka kalibrasi XPT2046 -- yang di LovyanGFX terikat
//     ke rotasi yang aktif SAAT kalibrasi dilakukan -- jadi tidak
//     match dengan rotasi runtime. Sumbu X/Y touch pun tertukar, dan
//     usapan FISIK ke atas di lock screen malah kebaca sebagai
//     gerakan menyamping oleh lockScreenInput() (yang cuma mengecek
//     dy = startY-ty), jadi swipe-to-unlock tidak pernah ter-trigger.
//  2. FIX (v9, versi final dipilih user): daripada menebak-nebak
//     orientasi tersimpan lebih dulu, setup() dibalikin PERSIS ke
//     urutan & default kode lama v6 yang sudah terbukti TIDAK PERNAH
//     kena bug ini: display.setRotation(1) (LANDSCAPE) di-hardcode
//     paling awal, SEBELUM boot sequence & kalibrasi dijalankan, dan
//     loadOrientPref() default-nya juga dibalikin ke LANDSCAPE (bukan
//     PORTRAIT lagi). Dengan begitu rotasi kalibrasi & rotasi default
//     runtime dijamin SELALU landscape, persis sama, tidak pernah
//     mismatch -- sama seperti v6. Kalau user mau pakai portrait,
//     tinggal ganti manual lewat Control Center kapan saja (App ini
//     sudah terbukti bisa ganti rotasi live tanpa perlu kalibrasi
//     ulang, jadi aman).
//
...
Orientation loadOrientPref() {
  Preferences p; p.begin("ui", true);
  // FIX v9: default orientasi dibalikin ke LANDSCAPE, sama seperti kode
  // lama v6 -- supaya selalu match dengan display.setRotation(1) yang
  // di-hardcode duluan di setup() sebelum kalibrasi touch dijalankan.
...
void setup(){
  ...
  // =============================================================
  // FIX v9: urutan di bawah ini SENGAJA dibalikin persis sama seperti
  // kode lama v6 yang sudah terbukti TIDAK PERNAH kena bug "lock
  // screen tidak bisa diusap ke atas setelah kalibrasi". Kuncinya:
  // rotasi di-hardcode ke LANDSCAPE DULU (sebelum boot sequence &
  // kalibrasi touch dijalankan), dan loadOrientPref() default-nya
  // juga LANDSCAPE -- jadi rotasi saat kalibrasi & rotasi default
  // runtime dijamin selalu sama, tidak pernah tertukar sumbu X/Y-nya.
  // Mau pakai portrait tinggal ganti manual di Control Center kapan
  // saja, itu tidak butuh kalibrasi ulang.
  // =============================================================
  display.setRotation(1); // landscape = orientasi default, sama seperti kode lama v6
  canvas.setPsram(true);
  ...
  display.setBrightness(brightness);
  if(display.touch())loadOrRunCalibration(); // kalibrasi SELALU terjadi di rotasi landscape ini
  loadTheme();
  initAppColors();
  drawColor=T().text;
  initSD();
  ...
  canvasApp.fillSprite(T().bg);
  Orientation savedOrient = loadOrientPref(); // default LANDSCAPE (lihat loadOrientPref())
