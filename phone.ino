...
void triggerGeminiAI() {
  ...
  // v36 FIX BESAR (dikonfirmasi user: AI Live gagal di PERCOBAAN PERTAMA,
  // padahal itu koneksi TLS PERTAMA di fitur itu -- bukan cuma "abis
  // dipake berkali-kali"): stack task ini SENDIRI (16384 byte) DIPOTONG
  // LANGSUNG dari RAM INTERNAL yg sama, PAS SEBELUM TLS sempat kebagian
  // apa-apa. Ini PERSIS temuan v30 dulu: largestBlock 47000 (sblm task
  // dibikin) jadi 31732 (SESUDAH task jalan, cuma gara2 stack-nya) --
  // beda 16384-nya SAMA PERSIS dgn ukuran stack yg diminta. Jadi
  // "kejadian pertanyaan ke-2 gagal" itu SEBENERNYA bukan soal "makin
  // lama makin fragmentasi" doang -- separuh masalahnya adalah stack
  // 16384 byte itu SENDIRI udah motong headroom yg tadinya cuma pas-
  // pasan buat TLS (~32KB). Di AI Live headroom-nya makin tipis lagi
  // krn baseline RAM device ini emang udah mepet dari awal boot (lihat
  // HWmonitor: cuma ~47KB bebas dr 162KB total) -- jd begitu stack
  // task ke-carve duluan, largestBlock yg tersisa buat TLS gampang
  // banget jatuh di bawah 20000.
  // FIX: turunin ukuran stack ke 12288 (msh JAUH lebih besar dr yg
  // beneran kepake -- HTTPClient/WiFiClientSecure/String/JSON parsing
  // di doGeminiHttpRequest() gak butuh sedekat itu ke 16KB, tapi msh
  // dikasih margin gede drpd langsung mepetin). Ini ngembaliin ~4KB
  // RAM internal balik ke pool yg sama dipake TLS. Kalau msh gagal
  // stlh ini, cek log Serial "[Gemini] Sisa stack task pas selesai:
  // ... byte" -- kalau angkanya msh gede byte (jauh dr 0), stack ini
  // AMAN diperkecil lebih jauh lagi drpd 12288.
...
void aiLiveStartTask(){
  ...
  // v36 FIX: sama kayak geminiTaskFunc (lihat catatan panjang di
  // triggerGeminiAI()), stack task ini SENDIRI dipotong dari RAM internal
  // yg SAMA dibutuhin TLS -- inilah kenapa AI Live bisa gagal di
  // PERCOBAAN PERTAMA (TLS koneksi #1, bukan #2/#3): begitu task ini
  // dibikin, largestBlock LANGSUNG turun sebesar ukuran stack yg
  // diminta, SEBELUM satu byte pun terpakai buat rekaman/HTTP. Diturunin
  // dari 16384 -> 13312 (msh dikasih extra dr punya geminiTaskFunc krn
  // di app ini ada tambahan buffer lokal int32_t rawBuf[512] ~2KB pas
  // proses rekam mic). Cek log "[AI Live] Sisa stack..." abis dipake
  // buat liat beneran kepake berapa.
  BaseType_t res=xTaskCreatePinnedToCore(aiLiveTaskFunc,"aiLiveTask",13312,NULL,1,&aiLiveTaskHandle,1);
  if(res!=pdPASS){
