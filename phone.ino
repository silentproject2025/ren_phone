...
void triggerGeminiAI() {
  ...
  if (largestBlock < 46000) {
    // v27 REVERT: balik ke 46000 (nilai aman asli sblm sesi ini) krn
    // client.setBufferSizes() gak valid dipakai (lihat catatan revert di
    // doGeminiHttpRequest()) -- kebutuhan RAM TLS-nya balik ke default
    // mbedTLS (~32KB), jd ambang batas longgar (20000) yg dipatok pas
    // masih pakai setBufferSizes() udah gak relevan lagi/bisa bikin
    // task tetep dibikin walau RAM-nya sbnrnya kurang -> crash beneran
    // drpd cuma nampilin pesan error yg rapi.
