...
bool aiLiveChatFromAudio(uint8_t* wavBuf, size_t wavLen, String& outTranscript, String& outReply){
  geminiApiKey.trim();
  if(geminiApiKey.length()==0 || geminiApiKey=="YOUR_GEMINI_API_KEY_HERE") return false;
  if(WiFi.status()!=WL_CONNECTED) return false;
  // FIX (bug "AI Live selalu gagal, cek RAM"): cek largestBlock internal DULU
  // ada di sini, tapi titik ini dipanggil DARI DALAM aiLiveTaskFunc, yaitu
  // DI DALAM task "aiLiveTask" yg stack 16KB-nya sendiri dialokasikan dari
  // RAM internal pas xTaskCreatePinnedToCore() dipanggil di aiLiveStartTask().
  // Artinya largestBlock yg diukur DI SINI SELALU sudah kepotong ~16KB duluan
  // oleh stack task itu sendiri -- SELALU gagal walau RAM sebenarnya cukup
  // (persis kasus yg sudah didokumentasikan di v34 punya AI Chat teks, lihat
  // catatan panjang di triggerGeminiAI()). Cek yg BENAR (dgn retry, SEBELUM
  // task dibikin) sekarang dipindah ke aiLiveStartTask() -- lihat di situ.
  ...
  HTTPClient http; http.setTimeout(20000); http.setConnectTimeout(15000);
  String url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash-lite:generateContent?key=" + geminiApiKey;
  if(!http.begin(client,url)){ heap_caps_free(payload); return false; }
  http.addHeader("Content-Type","application/json");
  http.addHeader("Connection","close");
...
bool aiLiveTextToSpeech(const String& text, uint8_t** outPcm, size_t* outPcmLen, uint32_t* outSampleRate){
  geminiApiKey.trim();
  if(geminiApiKey.length()==0) return false;
  if(WiFi.status()!=WL_CONNECTED) return false;
  if(heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL) < 20000) return false;
...
#define AILIVE_TASK_STACK 16384
...
void aiLiveStartTask(){
  ...
  // FIX (bug "AI Live selalu gagal dapat jawaban, cek WiFi/API key/RAM" --
  // padahal WiFi & API key udah bener, & RAM total di HWmonitor msh sehat
  // mis. 61%): cek largestBlock RAM INTERNAL harus dilakukan DI SINI, di
  // context loop() utama SEBELUM task "aiLiveTask" dibikin -- BUKAN di
  // dalam aiLiveChatFromAudio() spt sebelumnya (itu udah TERLAMBAT, krn
  // begitu xTaskCreatePinnedToCore() di bawah ini jalan, stack 16KB task
  // itu SENDIRI langsung motong largestBlock RAM internal yg cuma 162KB
  // total, jd cek yg dilakukan DI DALAM task nyaris SELALU gagal, konsisten
  // gagal terus tiap kali dipakai, entah ngomongnya sebentar atopun lama --
  // pola persis yg sudah pernah kejadian & difix v34 di AI Chat teks
  // (triggerGeminiAI()), tapi kelewat pas nulis app AI Live baru ini).
  //
  // Ambang di sini SENGAJA dilebihkan dr 20000 punya AI Chat teks --
  // ditambah AILIVE_TASK_STACK -- krn di sini kita HARUS nyisain jatah
  // buat stack task yg BELUM dibikin (bakal motong RAM internal begitu
  // xTaskCreatePinnedToCore jalan), SETELAH itu barulah TLS/mbedTLS di
  // dalam task butuh largestBlock >=20000 lagi spt biasa.
  const size_t MIN_BLOCK_TO_START = 20000 + AILIVE_TASK_STACK;
  size_t largestBlock = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL);
  for(int i=0; i<3 && largestBlock<MIN_BLOCK_TO_START; i++){
    delay(150);
    largestBlock = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL);
  }
  Serial.printf("[AI Live] Sebelum bikin task -> largestBlockInternal=%u (butuh>=%u) freePsram=%u\n",
                largestBlock, (unsigned)MIN_BLOCK_TO_START, ESP.getFreePsram());
  if(largestBlock < MIN_BLOCK_TO_START){
    aiLiveStatus="RAM internal kritis, tutup app lain / restart"; aiLiveError=true;
    showToast("RAM internal kritis, tutup app lain dulu"); vibError();
    needRedrawNow();
    return;
  }

  BaseType_t res=xTaskCreatePinnedToCore(aiLiveTaskFunc,"aiLiveTask",AILIVE_TASK_STACK,NULL,1,&aiLiveTaskHandle,1);
  if(res!=pdPASS){
