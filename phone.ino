...
#define AILIVE_TMP_WAV "/ai_live_reply.wav"
...
void aiLiveTaskFunc(void* parameter){
  ...
  bool ttsOk = sdReady && aiLiveTextToSpeech(reply, &pcmBuf, &pcmLen, &sr);

  if(!ttsOk){
    aiLiveStatus = !sdReady? "Jawaban siap (teks doang -- SD gak ada)"
                           : "Gagal bikin suara, jawaban cuma teks";
    aiLiveError = sdReady; // kalau emang gara2 gak ada SD, itu bukan "error" tak terduga
    if(pcmBuf) heap_caps_free(pcmBuf);
  } else {
    size_t wavTotalLen = WAV_HEADER_SIZE + pcmLen;
    uint8_t* wavOut=(uint8_t*)heap_caps_malloc(wavTotalLen, MALLOC_CAP_SPIRAM|MALLOC_CAP_8BIT);
    if(wavOut){
      memcpy(wavOut+WAV_HEADER_SIZE, pcmBuf, pcmLen);
      micWriteWavHeader(wavOut, (uint32_t)pcmLen, sr);
      File f = SD_MMC.open(AILIVE_TMP_WAV, FILE_WRITE);
      if(f){
        f.write(wavOut, wavTotalLen); f.close();
        mp3Audio->connecttoFS(SD_MMC, AILIVE_TMP_WAV); // playback jalan sendiri lewat mp3Audio->loop() yg SUDAH dipanggil tiap loop()
        aiLiveStatus="Ngomong...";
      } else {
        aiLiveStatus="Gagal simpan audio ke SD"; aiLiveError=true;
      }
      heap_caps_free(wavOut);
    } else {
      aiLiveStatus="PSRAM kurang buat bungkus WAV"; aiLiveError=true;
    }
