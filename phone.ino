...
bool aiLiveChatFromAudio(uint8_t* wavBuf, size_t wavLen, String& outTranscript, String& outReply){
  ...
  if(WiFi.status()!=WL_CONNECTED) return false;
  if(heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL) < 20000) return false; // sama ambang dgn doGeminiHttpRequest()
...
void aiLiveStartTask(){
  ...
  BaseType_t res=xTaskCreatePinnedToCore(aiLiveTaskFunc,"aiLiveTask",16384,NULL,1,&aiLiveTaskHandle,1);
  if(res!=pdPASS){
