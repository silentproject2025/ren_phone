...
bool doGeminiHttpRequest(const String& promptText, String& outResponse) {
  ...
  client.setInsecure();
  // v27 FIX: AKAR MASALAH error "RAM internal terlalu terfragmentasi/
  // rendah utk TLS" -- bukan krn heap_caps_malloc_extmem_enable() di
  // setup() gak ngefek (itu emang cuma nolong alokasi UMUM spt String/
  // HTTPClient, BUKAN buffer TLS-nya sendiri), tapi krn mbedTLS di balik
  // WiFiClientSecure DEFAULT minta buffer in+out masing2 16384 byte (jd
  // total 32KB) dr RAM INTERNAL (bukan PSRAM, gak ngikut extmem_enable).
  // Di ESP32-S3 yg RAM internalnya sudah kepake WiFi stack + sensor +
  // app lain, nyari 1 BLOK KONTIGU 32KB+overhead (makanya threshold cek
  // di triggerGeminiAI() dipatok 46000) gampang gagal walau total RAM
  // bebasnya sbnrnya masih banyak (tinggal kepecah2 alias fragmented).
  // setBufferSizes() bikin mbedTLS minta buffer JAUH lebih kecil --
  // gak ngebatasin ukuran JSON request/respons (mbedTLS otomatis kirim/
  // terima lewat beberapa TLS record kalau datanya lebih gede dr
  // buffer, cuma jd sedikit lebih banyak round-trip, BUKAN gagal).
  client.setBufferSizes(4096, 2048);
...
void triggerGeminiAI() {
  ...
  if (largestBlock < 20000) {
    // v27: ambang batas diturunin dr 46000 -> 20000 krn doGeminiHttpRequest()
    // skrg pakai client.setBufferSizes(4096,2048) (total ~6KB, dulu default
    // mbedTLS minta ~32KB). Angka 20000 tetep dikasih ruang aman utk overhead
    // TLS context/handshake lain di luar buffer in/out itu sendiri.
