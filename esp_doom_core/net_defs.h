...
struct _net_module_s
{
    ...
    doom_boolean (*InitClient)(void);

    // Initialize this module for use as a server

    doom_boolean (*InitServer)(void);
    ...
    doom_boolean (*RecvPacket)(net_addr_t **addr, net_packet_t **packet);
...
typedef struct 
{
    signed int latency;
    unsigned int seq;
    doom_boolean playeringame[NET_MAXPLAYERS];
