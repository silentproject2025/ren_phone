...
{
    atexit_func_t func;
    doom_boolean run_on_error;
    atexit_listentry_t *next;
};

static atexit_listentry_t *exit_funcs = NULL;

void I_AtExit(atexit_func_t func, doom_boolean run_on_error)
{
...
doom_boolean I_ConsoleStdout(void)
{
#ifdef _WIN32
...
static doom_boolean already_quitting = false;
...
#if ORIGCODE
    SDL_Quit();
    printf("Warning: abort with SDL Quit.\n");
    exit(-1);
#else
    printf("Warning: abort.\n");
    exit(-1);
#endif
}
...
doom_boolean I_GetMemoryValue(unsigned int offset, void *value, int size)
{
    static doom_boolean firsttime = true;
