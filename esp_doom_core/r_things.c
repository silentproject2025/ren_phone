...
//
void R_ShutdownThings (void)
{
    if (vissprites) { heap_caps_free(vissprites); vissprites = NULL; }
    vissprite_p = NULL;
}
...
void R_InitSprites (char** namelist)
{
    if (vissprites == NULL)
    {
        vissprites = (vissprite_t*)heap_caps_malloc(
            sizeof(vissprite_t) * MAXVISSPRITES,
            MALLOC_CAP_SPIRAM);

        if (!vissprites)
        {
            I_Error("No PSRAM for vissprites");
        }
    }
...
void R_ProjectSprite (mobj_t* thing)
{
    ...
    if ( (thing->frame&FF_FRAMEMASK) >= sprdef->numframes ){
        //return;
	    //I_Error ("R_ProjectSprite: invalid sprite frame    aqui      %i : %i ", thing->sprite, thing->frame);
    }
#endif
    //sprframe = &sprdef->spriteframes[ thing->frame & FF_FRAMEMASK];
    sprframe = &sprdef->spriteframes[thing->frame & FF_FRAMEMASK];

    if (thing->sprite == 68) return;
    if (sprframe->rotate)
    {
