...
#include "esp_bt.h"   // v21 FIX: buat esp_bt_controller_mem_release()
...
void setup(){
  ...
  // v21 FIX: proyek ini SAMA SEKALI gak pakai Bluetooth/BLE di manapun
  // (sudah dicek: gak ada BluetoothSerial/NimBLE/esp_bt lain selain baris
  // ini). Tapi ESP32-S3 defaultnya tetap MEREZERVASI RAM internal buat
  // controller BT/BLE sejak boot walau gak pernah diinisialisasi/dipakai
  // -- alokasi ini TIDAK otomatis lepas sendiri. Ini teknik resmi dari
  // ESP-IDF (esp_bt_controller_mem_release) buat ngelepas reservasi itu
  // balik ke heap internal, biar largestBlock RAM internal lebih lega
  // utk TLS Gemini. Ini KEMUNGKINAN BESAR penjelasan kenapa RAM internal
  // udah mepet bahkan di PERTANYAAN PERTAMA (langsung dari boot, blm
  // sempet buka app lain) -- beda dgn dugaan2 sebelumnya (riwayat AI
  // Chat, timing lwIP) yg keduanya udah kebukti gak relevan/gak cukup.
  // HARUS dipanggil di awal banget, SEBELUM ada kode lain yg mungkin
  // nyentuh BT (di proyek ini gak ada, jadi aman), dan cuma sekali sejak
  // boot (gak bisa di-undo/BT gak bisa dipakai lagi stlh ini di sesi yg
  // sama -- gapapa krn emang gak pernah dipakai).
